...
random.seed(66) # inshop
# random.seed(91) # consumer2shop

##### Consumer2Shop+ Object occlusion #####
...
save_path = '../dataset/Consumer2Shop_Small_Objectoccluded/'
if not os.path.exists(save_path):
    os.makedirs(save_path)

img_save_path = '../dataset/Consumer2Shop_Small_Objectoccluded/small_object_occluded_img/'
if not os.path.exists(img_save_path):
    os.makedirs(img_save_path)

mask_save_path = '../dataset/Consumer2Shop_Small_Objectoccluded/small_object_occluded_mask/'
if not os.path.exists(mask_save_path):
    os.makedirs(mask_save_path)

##### Consumer2Shop + Hand occlusion #####
# save_path = '../dataset/Consumer2Shop_Small_Handoccluded/'
# if not os.path.exists(save_path):
#     os.makedirs(save_path)
#
# img_save_path = '../dataset/Consumer2Shop_Small_Handoccluded/small_hand_occluded_img/'
# if not os.path.exists(img_save_path):
#     os.makedirs(img_save_path)
#
# mask_save_path = '../dataset/Consumer2Shop_Small_Handoccluded/small_hand_occluded_mask/'
# if not os.path.exists(mask_save_path):
#     os.makedirs(mask_save_path)
#
# data_root = '../dataset/Consumer2Shop/img/'
                    ...
                    if os.path.isdir(img):
                        ...
                        for name in imgs:
                            ...
                            if 'shop' in name: # don't occlude shop image
                                ...
                                cv2.imwrite(real_final_path, read_img)
                                continue
                            else: # only occlude consumer imageo
                                ...
                                if desired_size[0] >= H or desired_size[1] >= W:
                                    min_scale = min(H / desired_size[0], W / desired_size[1])
                                ...
                                canvas = np.zeros((H, W))
                                a, b = int(H * 0.4), int(H * 0.6)
                                c, d = int(W * 0.4), int(W * 0.6)
                                init_x, init_y = random.randint(a,b), random.randint(c,d)

                                end_row = init_x + resized_mask.shape[0] if init_x + resized_mask.shape[0] <= H else H
                                end_col = init_y + resized_mask.shape[1] if init_y + resized_mask.shape[1] <= W else W

                                mask_end_row = H - init_x if end_row >= H else resized_mask.shape[0]
                                mask_end_col = W - init_y if end_col >= W else resized_mask.shape[1]
